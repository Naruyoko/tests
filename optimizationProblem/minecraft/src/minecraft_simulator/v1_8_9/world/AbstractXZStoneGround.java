...
/**
 * A utility class for a world handling horizontal movement with the ground made
 * of full blocks.
 */
public abstract class AbstractXZStoneGround implements IXZMoveEntityHandler<AbstractXZPlayer> {
  ...
  public boolean hasAnyHorizontallyCollidingBoundingBoxes(XZAxisAlignedBB bb) {
    ...
    for (int x = minX; x < maxX; x++) {
      for (int z = minZ; z < maxZ; z++) {
        if (hasBlockAt(x, z) && Blocks.fullBlock.hasAnyCollidingBoundingBoxes(x, z, bb))
          return true;
      }
    }
    return false;
  ...
  public SimulationFlagsOut moveEntity(AbstractXZPlayer player, double x, double z, SimulationFlagsIn flagsIn,
    ...
    if (flagsIn.checkSneaking && flagsIn.onGround && flagsIn.isSneaking) {
      ...
      while (x != 0.0D && !hasAnyHorizontallyCollidingBoundingBoxes(
      ...
      while (z != 0.0D && !hasAnyHorizontallyCollidingBoundingBoxes(
      ...
      while (x != 0.0D && z != 0.0D && !hasAnyHorizontallyCollidingBoundingBoxes(
