...
/**
 * A utility class for a world handling horizontal movement with the ground and wall made of full blocks.
 */
public abstract class AbstractXZStoneGroundAndWall implements IXZMoveEntityHandler<AbstractXZPlayer> {
  ...
  public List<XZAxisAlignedBB> getHorizontallyCollidingGroundBoundingBoxes(XZAxisAlignedBB bb){
    ...
    for (int x=minX;x<maxX;x++){
      for (int z=minZ;z<maxZ;z++){
        if (hasGroundBlockAt(x,z)) Blocks.fullBlock.addCollisionBoxesToList(x, z, bb, list);
      }
    }
    return list;
  }
  ...
  public boolean hasAnyHorizontallyCollidingGroundBoundingBoxes(XZAxisAlignedBB bb){
    ...
    for (int x=minX;x<maxX;x++){
      for (int z=minZ;z<maxZ;z++){
        if (hasGroundBlockAt(x,z)&&Blocks.fullBlock.hasAnyCollidingBoundingBoxes(x,z,bb)) return true;
      }
    }
    return false;
  }
  ...
  public List<XZAxisAlignedBB> getHorizontallyCollidingWallBoundingBoxes(XZAxisAlignedBB bb){
    ...
    for (int x=minX;x<maxX;x++){
      for (int z=minZ;z<maxZ;z++){
        if (hasWallBlockAt(x,z)) Blocks.fullBlock.addCollisionBoxesToList(x, z, bb, list);
      }
    }
    return list;
  }
  ...
  public boolean hasAnyHorizontallyCollidingWallBoundingBoxes(XZAxisAlignedBB bb){
    ...
    for (int x=minX;x<maxX;x++){
      for (int z=minZ;z<maxZ;z++){
        if (hasWallBlockAt(x,z)&&Blocks.fullBlock.hasAnyCollidingBoundingBoxes(x,z,bb)) return true;
      }
    }
    return false;
  }
  ...
  public SimulationFlagsOut moveEntity(AbstractXZPlayer player,double x,double z,SimulationFlagsIn flagsIn,SimulationFlagsOut flagsOut){
    ...
    if (flagsIn.checkSneaking&&flagsIn.onGround&&flagsIn.isSneaking){
      ...
      while (x!=0.0D&&!hasAnyHorizontallyCollidingGroundBoundingBoxes(XZAxisAlignedBB.copyOffset(workingBoundingBox,player.boundingBox,x,0.0D))){
