...
function compression1(s,passes=4){
  ...
  if (dict.length){
    ...
    r="_="+options.quoteType+escapeNewLinesAndQuotes(s+dict.map(function(s){return s[1]+s[0];}).join(""))+options.quoteType+"\nfor Y in"+options.quoteType+escapeNewLinesAndQuotes(dict.map(function(s){return s[1];}).reverse().join(""))+options.quoteType+":_=_.split(Y);_=_.pop().join(_)\nexec(_)";
  }else r=s;
  console.log(r);
  return r;
}
